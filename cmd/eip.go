...
func init() {
	...
	eipCmdAssign.Flags().IntVar(&eipAssignBandwidthSize, "bandwidth-size", 5, "Specifies the bandwidth size. The value ranges from 1 Mbit/s to 300 Mbit/s by default. (The specific range may vary depending on the configuration in each region. You can see the bandwidth range of each region on the management console.) This parameter is mandatory when share_type is set to PER. This parameter will be ignored when share_type is set to WHOLE with an ID specified.")
	eipCmdAssign.Flags().StringVar(&eipAssignBandwidthShareType, "bandwidth-share-type", "PER", "Specifies the bandwidth type. Possible values are PER (Dedicated bandwidth) and WHOLE (Shared bandwidth). If this parameter is set to WHOLE, the bandwidth ID must be specified")
